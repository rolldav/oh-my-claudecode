...
import { normalizeClaudeLaunchArgs, extractOmcLaunchFlags, runClaude, launchCommand, } from '../launch.js';
...
describe('runClaude', () => {
    ...
    it('creates a tmux session when outside tmux (default)', () => {
        ...
        const tmuxCall = calls.find((c) => c[0] === 'tmux');
...
describe('launchCommand with --no-tmux', () => {
    ...
    it('calls claude directly when noTmux is true', async () => {
        ...
        const claudeCall = execFileSync.mock.calls.find((c) => c[0] === 'claude');
        expect(claudeCall).toBeDefined();
        expect(claudeCall[1]).toContain('--verbose');
        // Must NOT create a tmux session
        const tmuxCall = execFileSync.mock.calls.find((c) => c[0] === 'tmux');
